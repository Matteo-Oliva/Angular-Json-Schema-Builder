...
export class JsonSchemaService {
    ...
    newXOf(type, key, props?, parent?) {
        var newXOf = { ...{}, ...this.additional['for' + type] };
        this._id_ += 1;
        newXOf.__ID__ = '$model' + this._id_;
        newXOf._key = key;
    ...
    schema2obj(schema, key = null, required = null, isRoot = null, modelObjs = null) {
        ...
        for (let x = 0; x < schema.type.length; x++) {
            ...
            switch (type) {
                ...
                case 'AnyOf':
                case 'AllOf':
                    const newOfModel = this.copy(this.newXOf(type, key, null, parent));
                    if (obj && obj._type && obj._type.length > 0) {
                        obj._type.push(newOfModel._type[0]);
                        this.copyProps(obj, newOfModel);
                    } else {
                        obj = newOfModel;
                    }
                    if (schema.description) {
